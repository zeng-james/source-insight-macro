/******************************************************************************
*
*  Copyright (C), James Zeng, james.zeng@hotmail.com
*
*******************************************************************************
*  File Name     : efficient.em
*  Version       : Initial Draft
*  Author        : James Zeng
*  Created       : 2022/3/17
*  Last Modified :
*  Description   : This is a Source Insight extended macro module to make
...
macro AutoExpand()
{
    ...
    //Msg("szLine1:@szLine1@, szLine:@szLine@")
...
macro ExpandProcEN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel)
{
    ...
    {   
        ...
        {
            Msg("The right margin is small, Please use a new line")
            stop 
        }
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if (szCmd == "switch")
    {
        ...
        while(nIdx < nSwitch)
        {
            InsBufLine(hbuf, ln , "@szLine1@    " # "case # :")
            InsBufLine(hbuf, ln + 1    , "@szLine1@    " # "    " # ";")
            InsBufLine(hbuf, ln + 2, "@szLine1@    " # "    " # "break;")
            InsBufLine(hbuf, ln + 3, "")
            ln = ln + 4
            nIdx = nIdx + 1
        }

        InsBufLine(hbuf, ln, "@szLine1@    " # "default:")
        InsBufLine(hbuf, ln + 1, "@szLine1@    " # "    " # ";")
        InsBufLine(hbuf, ln + 2, "@szLine1@    " # "    " # "break;")
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}")
    ...
    else if (szCmd == "name")
    {
        ...
        szMyName = Ask("Enter your name:")
        setreg(MYNAME, szMyName)
        Msg("Your name input successfully!")
        Beep()
        return
    }
    else
...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    szCopyright = GetReg("COPYRIGHT")
    if (szCopyright == "")
    {
    	szCopyright = "James Zeng, james.zeng@@hotmail.com"
    }
    GetFunctionList(hbuf,hnewbuf)
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "*")
    InsBufLine(hbuf, ln + 2,  "*  Copyright (C), @szCopyright@")
    InsBufLine(hbuf, ln + 3,  "*")
    InsBufLine(hbuf, ln + 4,  "*******************************************************************************")
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), @szCopyright@")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
...
macro ConfigureSystem()
{
    ...
    szCopyright = ASK("Please input your copyright owner")
    if(szCopyright == "#")
    {
       SetReg ("COPYRIGHT", "")
    }
...
macro ShowHelp(hbuf, ln)
{
    ...
    i = i+1
    InsBufLine(hbuf, ln + i, " *  abe            auto insert add begin and end description for assigned PN comment")
    i = i+1
    InsBufLine(hbuf, ln + i, " *  mbe            auto insert modify begin and end description for assigned PN comment")
    i = i+1
    InsBufLine(hbuf, ln + i, " *  dbe            auto insert delete begin and end description for assigned PN comment")
    i = i+1
    InsBufLine(hbuf, ln + i, " *")
    i = i+1
